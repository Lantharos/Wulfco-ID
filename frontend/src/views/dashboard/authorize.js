...
const Authorize = (props) => {
    ...
    const loadPage = () => {
        ...
        if (cookies.load("session_id")) {
            fetch(`${api_url}/oauth?app=${client_id}&id=${encodeURIComponent(cookies.load("id"))}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'W-Auth': hmac(cookies.load('token'), cookies.load('secret')).toString(),
                ...
                res.json().then((data) => {
                    if (data.success) {
                        if (!data.app) { toast.error('Invalid app', { theme: 'dark', autoClose: 2000 }); return; }
